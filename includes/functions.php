...
ini_set('error_log', '/home/dtrmfslh/public_html/functions_debug.log');
...
/**
 * Create loan installments - FIXED VERSION
 */
function createLoanInstallments($loan_id, $weekly_installment, $number_of_weeks) {
    global $conn;
    
    error_log("📋 CREATING INSTALLMENTS FOR LOAN: " . $loan_id);
    error_log("   Weekly Installment: " . $weekly_installment);
    error_log("   Number of Weeks: " . $number_of_weeks);
    
    try {
        // First, let's check the loan_installments table structure
        error_log("🔍 Checking loan_installments table structure...");
        
        for ($i = 1; $i <= $number_of_weeks; $i++) {
            $due_date = date('Y-m-d', strtotime("+$i weeks"));
            
            // Use parameterized query with status as parameter
            $installment_sql = "INSERT INTO loan_installments (loan_id, installment_number, due_date, amount, status) 
                               VALUES (?, ?, ?, ?, ?)";
            
            error_log("   Creating installment $i: " . $installment_sql);
            error_log("   Parameters: loan_id=$loan_id, installment_number=$i, due_date=$due_date, amount=$weekly_installment, status='pending'");
            
            $installment_stmt = $conn->prepare($installment_sql);
            if (!$installment_stmt) {
                error_log("❌ INSTALLMENT PREPARE ERROR: " . $conn->error);
                throw new Exception("Installment prepare failed: " . $conn->error);
            }
            
            // Use safe status value
            $installment_status = 'pending';
            
            $bind_result = $installment_stmt->bind_param("iisds", 
                $loan_id, 
                $i, 
                $due_date, 
                $weekly_installment, 
            ...
            if (!$bind_result) {
                error_log("❌ INSTALLMENT BIND ERROR: " . $installment_stmt->error);
                throw new Exception("Installment bind failed: " . $installment_stmt->error);
            }
            
            $execute_result = $installment_stmt->execute();
            if (!$execute_result) {
                error_log("❌ INSTALLMENT EXECUTE ERROR: " . $installment_stmt->error);
                throw new Exception("Installment execute failed: " . $installment_stmt->error);
            }
            
            error_log("✅ Installment $i created successfully");
        }
        
        error_log("🎉 ALL INSTALLMENTS CREATED SUCCESSFULLY FOR LOAN: " . $loan_id);
        return true;
...
function checkAndCreateLoanTables() {
    ...
    error_log("🔍 Checking and creating loan tables...");
    ...
    if ($updated_result->num_rows == 0) {
        ...
        error_log("✅ Added updated_by column to loans table");
    }
    ...
    if ($result->num_rows == 0) {
        ...
        $conn->query($alter_sql);
        error_log("✅ Added missing columns to loans table");
    }
    ...
    if ($due_date_result->num_rows == 0) {
        ...
        error_log("✅ Added due_date column to loan_installments table");
    }
    
    // Check if loan_installments table status column is correct
    $check_status_sql = "SHOW COLUMNS FROM loan_installments LIKE 'status'";
    $status_result = $conn->query($check_status_sql);
    
    if ($status_result->num_rows > 0) {
        $status_data = $status_result->fetch_assoc();
        error_log("📊 loan_installments status column: " . $status_data['Type']);
    }
}
...
function ensureDatabaseColumns() {
    ...
    error_log("🔍 Ensuring database columns exist...");
    ...
    $required_columns = [
        ...
        'loan_installments' => [
            ...
            'status' => "ALTER TABLE loan_installments MODIFY status ENUM('pending','paid','overdue','completed','partial') DEFAULT 'pending'"
        ]
    ];
